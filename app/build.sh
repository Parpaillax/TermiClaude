#!/usr/bin/env bash
# Build de l'app barre de menus TermiClaude et assemblage du bundle .app.
...
BIN="$HERE/.build/release/TermiClaude"
APP="$HERE/TermiClaude.app"

echo "==> assemblage $APP"
mkdir -p "$APP/Contents/MacOS" "$APP/Contents/Resources"
cp -f "$BIN" "$APP/Contents/MacOS/TermiClaude"
...
if [ -d "$HERE/.build/release/TermiClaude_TermiClaude.bundle" ]; then
  cp -Rf "$HERE/.build/release/TermiClaude_TermiClaude.bundle" "$APP/Contents/Resources/"
fi
...
if [ -e "$APP/Contents/MacOS/TermiClaude_TermiClaude.bundle" ]; then
  mkdir -p "$HERE/.build/stale"
  mv "$APP/Contents/MacOS/TermiClaude_TermiClaude.bundle" "$HERE/.build/stale/" 2>/dev/null || true
fi
